...
import { TableInput } from "../types/inputTypes";
import { TableUpdate } from "../types/updateTypes";
...
export const tableResolvers = {
  ...
  Mutation: {
    addTable: authAsync(async (root, args: { tableInput: TableInput }) => {
    ...
    updateTable: authAsync(async (root, args: { tableUpdate: TableUpdate }) => {
      const { id, ...restProp } = args.tableUpdate;
      return prisma.table.update({ where: { id: Number(id) }, data: { ...restProp } })
    }, ['ADMIN']),

    deleteTable: authAsync(async (root, args: { id: string }) => {
      const { id } = args;
